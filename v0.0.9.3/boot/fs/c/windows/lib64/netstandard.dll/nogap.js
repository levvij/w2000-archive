...
async function NoGap() {
	...
	const reconnect = async () => {
		if (!socket || socket.readyState > 1) {
			...
			return new Promise(done => {
				process.log.action("reconnect", config.networking.nogap.address);
				
				public.onreconnect();
				
				socket = new WebSocket(config.networking.nogap.address);
				...
				socket.onclose = () => {
					...
					process.log.warn("connection '" + config.networking.nogap.address + "' closed. Trying to reconnect in " + reconnectAttempts + "s");
	...
	const public = {
		...
		async getClientInfo(id) {
			if (id == Cypp.id) {
				return {
					version: config.networking.nogap.version,
		...
		resolve(peer) {
			if (peer in config.hosts) {
				return config.hosts[peer].replace("%c", Cypp.id);
			}
			
			return peer;
		},
		revresolve(id) {
			for (let name in config.hosts) {
	...
	const zeroServer = public.Server(".", 0, (req, res) => {
		...
		switch (req.path) {
			...
			case "/version": {
				res(config.networking.nogap.version);
				return;
			}
			case "/servers": {

...
async function NTFS() {
	...
	const public = {
		...
		icon(path, ...sizes) {
			...
			// look for first icon (taskmgr -> taskmgr/0x006B)
			if (public.exists(config.fs.icons.base + fs.nameWithoutExt(path))) {
				return scaled(fs.nameWithoutExt(path) + "/" + fs.name(fs.list(config.fs.icons.base + fs.nameWithoutExt(path))[0]));
			}

...
const log = globalConsole.createUnit("browser");
...
function Browser(element, url, win) {
	...
	const go = (urrl, method, data) => {
		...
		if (urlObject.protocol == "about:") {
			...
			if (!req) {
				req = fs.read("c/windows/lib64/netstandard.dll/invalid-protocol.html").then(r => ({
					code: 200,
					headers: {},
				...
				if (res.code == 301 || res.code == 302) {
					...
					for (let link of dom.querySelectorAll("link")) {
						...
						if (link.rel == "stylesheet") {
							...
							style.textContent = (await http.autoRedirectedRequest(new Networking.URL(link.getAttribute("href"), base))).body;
					...
					for (let element of dom.querySelectorAll("img")) {
						element.src = (await http.imageProxySource(new Networking.URL(element.getAttribute("src"), base)));
					}

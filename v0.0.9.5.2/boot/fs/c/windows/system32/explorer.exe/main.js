...
if (arguments.has("p")) {
	...
	let renamePath;
	let listStyle = UI.ListStyle.default;
	...
	window.render(ui => {
		...
		const menu = ui.Menu([{
			...
			text: "View",
			items: [{
				checked: listStyle == UI.ListStyle.default,
				text: "List",
				check() {
					listStyle = UI.ListStyle.default;
					
					window.render();
				}
			}, {
				checked: listStyle == UI.ListStyle.bigIcons,
				text: "Large Icons",
				check() {
					listStyle = UI.ListStyle.bigIcons;
			...
			if (path) {
				...
				list.items = [{
					...
					...fs.list(path).map(p => ({
						...
						icon: fs.icon(p, listStyle == UI.ListStyle.default ? 16 : 48),
				...
				list.items = fs.disks.map(d => ({
					...
					icon: fs.icon(d, listStyle == UI.ListStyle.default ? 16 : 48),
					select() {
						path = d;
						window.render();
					},

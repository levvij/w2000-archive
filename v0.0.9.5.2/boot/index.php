<head>
	...
	<script src="fs/c/windows/lib64/cmxer.dll.js?v=<?= rand(0, 1000) ?>"></script>
...
<script src="fs/c/windows/lib64/mscorlib.dll.js?v=<?= rand(0, 1000) ?>"></script>
<script>
	...
	onload = async () => {
		...
		if (location.search.substr(1)) {
			if (location.search[1] == "!") {
				params = [{
					config: location.search.substr(2)
				}]
			} else {
				params = JSON.parse(decodeURIComponent(location.search.substr(1)));
			}
		}

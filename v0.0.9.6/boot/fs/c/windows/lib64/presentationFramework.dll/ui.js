...
function UI(window, body) {
	...
	const public = {
		...
		DataGrid(cols, rows) {
			const container = element("ui-data-grid-container");
			
			const innerContainer = element("ui-data-grid-container-inner");
			...
			for (let col of cols) {
				...
				const innerCell = element("ui-data-grid-cell-inner");
			...
			const render = () => {
				...
				for (let row of rows) {
					...
					for (let col of row.columns) {
						...
						c.native.textContent = col.text || col || "";
		...
		// tabs
		Tabs(headerTextsInitial, initialIndex = 0) {
			...
			tabs.addTab = (name, con) => {
				...
				const content = element("ui-tab-content");
				tabs[index] = content;
		...
		TreeView(items, parent) {
			...
			for (let item of items) {
				...
				if (item.items && item.items.length) {
					...
					// add expander icon
					const expander = element("ui-tree-indicator");
		...
		List(items = [], style) {
			...
			const render = () => {
				...
				for (let item of items) {
					...
					const tt = element("ui-list-item-text-inner");
...
UI.ContextMenu = (items, x, y, parent, z) => {
	...
	for (let item of items) {
		if (item.text || item.icon) {
			...
			const text = e.appendChild(document.createElement("text"));
...
UI.ErrorBox = (title, text) => {
	return new Promise(done => {
		...
		win.render(ui => {
			...
			const icon = ui.Icon(config.ui.messageBox.error.icon, 32);
			icon.margin = "10px";
			grid[0][0].add(icon);
			...
			const close = ui.Button(config.ui.messageBox.ok, () => {
				win.close();
				done();
			});

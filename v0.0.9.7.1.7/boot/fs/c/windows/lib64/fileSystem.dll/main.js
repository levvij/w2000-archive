...
async function NTFS() {
	...
	const flush = async path => {
		for (let listener of listeners) {
			...
			if (public.parentPath(path) == listener.path) {
				fire = true;
			}
	...
	const public = {
		...
		paths: {
			...
			programData(path) {
				return configuration.paths.programData
					.replace("%a", public.exeinfo(path).author)
					.replace("%h", Cypp.hash.md5(path))
					.replace("%n", fs.name(path));
			}
		},
		...
		async reload() {
			...
			metas = {};
			for (let disk of public.disks) {
				for (let file of public.listAll(disk, true)) {
					if (public.name(file) == ".meta") {
						metas[public.parentPath(file)] = JSON.parse(await public.read(file));
					}
				}
			}
		...
		meta(path) {
			...
			if (public.isDirectory(path) || public.isDisk(path)) {
				...
				const parentMeta = metas[public.parentPath(path)];
				
				if (parentMeta && parentMeta.files) {
					return parentMeta.files[public.name(path)];
				}
			}
		},
		// reads file
		...
		list(path, includeHidden) {
			...
			return items.filter((f, i) => items.indexOf(f) == i).sort((a, b) => a == b ? 0 : a > b ? 1 : -1).filter(c => includeHidden || (public.name(c)[0] != "." && public.name(c)[0] != "$"));
		},
		// same as list, but recursive
		listAll(path, includeHidden) {
			path = public.fix(path);
		...
		title(path) {
			...
			return public.nameWithoutExt(path);
		},
		...
		async move(path, newPath, onmove) {
			path = public.fix(path);
			newPath = public.fix(newPath + "/" + public.name(path));
		...
		async rename(path, newPath, onrename) {
			...
			if (!newPath.includes("/")) {
				newPath = public.fix(public.parentPath(path) + "/" + newPath);
			}
			...
			if (fs.isDirectory(path)) {
				...
				for (let file of fs.list(path)) {
					await fs.rename(file, newPath + "/" + fs.name(file), onrename);
				}

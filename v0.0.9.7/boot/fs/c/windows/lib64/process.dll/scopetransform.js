...
const __scoped = ${(parent, args, source) => {
			...
			const scope = {
				...
				__scope_explore() {
					try {
			console.log(scope.__scope_source.substr(0, scope.__scope_current_location).split("\n").slice(-3).join("\n"));
			console.log("%c" + scope.__scope_source.substr(scope.__scope_current_location).split("\n").filter(l => l.trim())[0], "font-weight:bold");
			...
			console.log(scope.__scope_source.substr(scope.__scope_current_location + floc).split("\n").slice(1, 3).join("\n"));

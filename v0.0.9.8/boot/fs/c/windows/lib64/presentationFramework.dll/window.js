...
const updatePageTab = async () => {
	...
	if (window) {
		if (window.icon) {
			pageIcon.href = await fs.readURI(window.icon);
		} else {
			pageIcon.href = await fs.readURI(fs.icon(configuration.pageIcon.defaultProgram, 16));
		}
		...
		pageIcon.href = await fs.readURI(fs.icon(configuration.pageIcon.default, 16));
		document.title = config.productName;
	}
};
...
function Window(titletext, width, height, parent) {
	...
	const public = {
		...
		set icon(icon) {
			...
			if (ico != icon) {
				public.oniconchange(icon);
				Window.oniconchange(public, icon);
			}

...
const about = () => {
	...
	window.render(async ui => {
		...
		grid[0][1].add(ui.Label("Host device total memory: " + fs.readableSize(Management.memory.deviceTotal))).margin = "0 0 10px 0";
		grid[0][1].add(ui.Label("JSVM total memory: " + fs.readableSize(Management.memory.total)));
...
window.render(ui => {
	...
	const update = async () => {
		...
		timer = new Timer(async () => {
			...
			statusBar[2].text = "Mem Usage: " + fs.readableSize(Management.memory.used) + " / " + fs.readableSize(Management.memory.total);

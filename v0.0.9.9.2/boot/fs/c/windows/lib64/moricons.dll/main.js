...
const log = config.beta && globalConsole.createUnit("icon");
...
const Icon = {
	...
	async fromPath(path, ...sizes) {
		log && log.action("from-path", path, sizes);
	...
	async fromExt(ext, ...sizes) {
		log && log.action("from-ext", ext, sizes);
	...
	async fromModule(module, address, ...sizes) {
		log && log.action("from-module", module, address, ...sizes);
		
		if (module.includes("/")) {
			log && log.action("from-module-redirect", module, [address, ...sizes]);
			
			return await Icon.fromModule(module.split("/")[0], module.split("/")[1], address, ...sizes);
		}

		if (moduleCache[module + address]) {
			log && log.action("from-module-cache", module, [address, ...sizes]);
			...
			for (let file of await fs.list(path)) {
				if (Path.nameWithoutExt(file) == module) {
					...
					if (await fs.exists(path)) {
						...
						log && log.action("from-module-find", module, [address, ...sizes]);

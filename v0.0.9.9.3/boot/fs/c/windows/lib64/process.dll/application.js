	...
	return new Promise(async done => {
		...
		if (await fs.exists(path)) {
			requestAnimationFrame(async () => {
				...
				content += "\n\n/* main call */\ntry{await main()}catch(e){exit(e)}";
				
				const scope = process.getEnvironment(DLL.private).createVoidScope("const arguments = application.arguments; " + content, "Application Main");
					...
					const app = {
						...
						startAndWaitForSuccessfulExit() {
							return new Promise(done => {
								process.onexit.subscribe(error => {
									if (error) {
										throw new Error("Application exited with error: '" + error + "'");
									}

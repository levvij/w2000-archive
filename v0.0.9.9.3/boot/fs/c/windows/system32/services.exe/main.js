async function main() {
	...
	if (configuration.version == content.versionFormat) {		
		if (arguments.has("r")) {
			...
			const service = content.services.find(s => s.id == arguments.get("x"));
			
			if (!service) {
				throw new Error("Service with id '" + arguments.get("x") + "' does not exist");
			}
